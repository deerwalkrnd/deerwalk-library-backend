    ...
    for row in rows:
        ...
        if "user_metadata" in row and isinstance(row["user_metadata"], str):
            try:
                ...
                raise LibraryException(status_code=500, code=ErrorCode.INVALID_FIELDS, msg="for some reason the server was not able to JSON Decode metadata field from csv.")
