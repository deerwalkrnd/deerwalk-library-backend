...
from app.modules.bookmarks.domain.repository.bookmark_repository_interface import BookmarkRepositoryInterface
...
class CheckBookmarkByBookIdUseCase():
    ...
    async def execute(self, book_id:int, user_id:str) -> bool:
        bookmark = await self.bookmark_repository.find_one(obj=Bookmark(user_id=user_id,book_id=book_id))
        if bookmark:
            return True
        else:
            return False